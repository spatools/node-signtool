...
function signtool(): string {
    ...
    switch(process.arch) {
        case "ia32":
            return ((<any>signtool).result = path.join(__dirname, "bin", "x86", "signtool.exe"));

        case "x64":
            return ((<any>signtool).result = path.join(__dirname, "bin", "x64", "signtool.exe"));
